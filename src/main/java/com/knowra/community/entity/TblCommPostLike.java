...
@Entity
@Table(
        name = "TBL_COMM_POST_LIKE",
        catalog = "KNOWRA_COMMUNITY",
        uniqueConstraints = @UniqueConstraint(
                name = "UK_COMM_POST_LIKE_USR",
                columnNames = {"COMM_POST_SN", "USER_SN"}
        )
)
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class TblCommPostLike {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "COMM_POST_LIKE_SN")
    @Comment("좋아요 SN (PK)")
    private Long commPostLikeSn;
